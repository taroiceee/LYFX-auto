...
def list_windows(visible_only=True):
    ...
    user32.EnumWindows(ctypes.WINFUNCTYPE(wintypes.BOOL, wintypes.HWND, ctypes.POINTER(ctypes.c_int))(enum_callback), None)
    return windows
...
def post_text(hwnd, text):
    ...
    for char in text:
        ...
        if char.isupper():
            # 大写字母需要先按Shift
            post_key(hwnd, VK_SHIFT)
            vk_code = ord(char)
            post_key(hwnd, vk_code)
            post_key(hwnd, VK_SHIFT)  # 释放Shift
        else:
            char_upper = char.upper()
            vk_code = ord(char_upper) if char_upper.isalpha() else None
...
def main():
    ...
    # 尝试多种可能的标题匹配
    possible_titles = ["无标题 - 记事本", "记事本", "新建文本文档", ".* - 记事本","记事本 - Notepad"]
    main_hwnd = None
    ...
    if not edit_hwnd:
        ...
        print(f"找到编辑区子窗口：{edit_hwnd}")
    
    # 纯后台操作演示
    print("\n开始后台操作...")
    
    # 1. 输入文本
    post_text(edit_hwnd, "这是纯后台输入的文本！\n")
    post_text(edit_hwnd, "不影响你正在操作的其他窗口哦～\n")
    time.sleep(1)
    
    # 2. 鼠标点击（在记事本客户区坐标 (100, 50) 位置）
    post_mouse_click(edit_hwnd, 100, 50)
    time.sleep(1)
    
    # 3. 继续输入
    post_text(edit_hwnd, "点击后继续输入的内容\n")
    time.sleep(1)
    
    # 4. 右键点击
    post_mouse_click(edit_hwnd, 100, 50, is_right=True)
    time.sleep(1)
